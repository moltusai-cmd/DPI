...
DPI replaces stochastic initialization with a **Sequential Bootstrapping** pipeline. Unlike global initialization methods, PID-14.1 treats the network as a dynamic flow, sculpting each layer using the real-time spectral signatures of the preceding manifold.
...
### 3.2 Sequential Manifold Sculpting
Rather than projecting global statistics across all layers, PID-14.1 employs an iterative sculpting process. For each layer $l \in [0, L-1]$:
...
The Value projections $W_v$ are constrained to a low-rank variety by applying aggressive spectral compression ($\gamma_v \approx 0.4 \gamma_{base}$). This forces the model to encode information along dominant principal components (PC1 dominance), facilitating stable value propagation.
...
To prevent gradient asphyxia at billion-parameter scales:
