...
def run_once(run_id):
    ...
    # Sweet Spot Point: ZW=1.1, SG=0.55, MA=0.50
    initialize_pid8(model, SimpleLoader(train_loader), zipf_warp=1.1, spectral_gamma=0.55, morph_alpha=0.50)
    ...
    print(f"Run {run_id} Complete | Avg Loss: {avg_loss:.4f}")
    return avg_loss

if __name__ == "__main__":
    print("Starting Double Sweet-Spot Verification...")
    res1 = run_once(1)
    res2 = run_once(2)
    print(f"\nVerification Results: Mean Loss = {(res1+res2)/2:.4f}")
