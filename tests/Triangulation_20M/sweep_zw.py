...
def run_zw_experiment(zw):
    ...
    # Fixed SG and MA at Resonance Point
...
if __name__ == "__main__":
    zw_values = [1.0, 1.1, 1.2, 1.3, 1.4]
    print(f"Starting ZW Sweep (Fixed SG=0.35, MA=0.35)...")
    
    results = {}
    for zw in zw_values:
        loss = run_zw_experiment(zw)
        print(f"  ZW: {zw:.1f} => Avg Loss: {loss:.4f}")
        results[zw] = loss
        
    print("\nSweep Complete.")
    best_zw = min(results, key=results.get)
    print(f"Best ZW in this range: {best_zw} with Loss {results[best_zw]}")
