...
def run_harmonic_experiment(ratio_name, K, gamma):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_file = "wiki.train.raw"
    vocab = build_vocab(train_file)
    
    zw = K * gamma
    ma = gamma # Assuming 1:1 ratio for Morph Alpha for now
...
if __name__ == "__main__":
    ratios = {
        "Square (4:1:1)": 4.0,
        "Circular (Pi:1:1)": math.pi,
        "Pyramidal (3:1:1)": 3.0,
        "Golden (Phi:1:1)": 1.618
    }
    gammas = [0.30, 0.35, 0.40]
    
    print(f"Starting Harmonic Ratio Sweep...")
    results = []
    
    for name, K in ratios.items():
        for g in gammas:
            loss = run_harmonic_experiment(name, K, g)
            print(f"  {name} | Gamma: {g:.2f} | (ZW: {K*g:.3f}) => Loss: {loss:.4f}")
            results.append({'ratio': name, 'K': K, 'gamma': g, 'loss': loss})
