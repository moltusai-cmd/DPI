...
def train():
    ...
    # Re-instantiate model with actual vocab size (though we used 16384 in plan)
    ...
    # PID-8 Initialization requires a sample batch
    class SimpleLoader:
        def __init__(self, dl): self.dl = dl
        def __iter__(self):
            for x, y in self.dl: yield x.to(device)
            
    print("Starting PID-8 Initialization...")
