...
class FastArxivDataset(Dataset):
    def __init__(self, tokenizer, seq_len=128, max_lines=15000):
...
def run_session(name, loader, device, options, total_steps=1000):
    ...
    cfg = dict(
        vocab_size=vocab_size, d_model=320, n_heads=5, n_layers=8, d_mlp=1280,
        use_rope=True, use_mup_attn=True, use_mup_readout=True
    )
    model = PID8Transformer(**cfg).to(device)
    
    # Official muP Shape Registration
    base_cfg = cfg.copy(); base_cfg['d_model'] = 160; base_cfg['d_mlp'] = 640
    ...
    if options.get('use_dpi', False):
        ...
        # ELITE CANONICAL muP Initialization
    ...
    # Optimizer - Official muP Transfer
...
def main():
    ...
    # Run the Elite Duel
    res_fusion = run_session("DPI_MUP_FUSION_V16_3_1", loader, device, {'use_dpi': True, 'base_lr': 1e-4})
    res_mup = run_session("PURE_MUP_OFFICIAL_ELITE", loader, device, {'use_dpi': False, 'base_lr': 1e-4})
    
    print("\n" + "="*80)
    print(f"🏆 ELITE DUEL: DPI v16.3.1 VS ELITE CANONICAL muP")
    print("="*80)
    print(f"{'Step':<10} | {'Pure muP (Elite)':<15} | {'DPI-muP Fusion':<15} | {'Delta':<10}")
    print("-" * 80)
