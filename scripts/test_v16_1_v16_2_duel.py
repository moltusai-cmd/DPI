...
def run_session(mode, loader, val_loader, device):
    print(f"\n🚀 Starting Session: DPI {mode.upper()} (Zero-Wait: {'YES' if mode=='v16.2' else 'NO'})")
    torch.manual_seed(42)
    model = PID8Transformer(vocab_size=16384, d_model=320, n_heads=5, d_mlp=1280, n_layers=8).to(device)
    
    # We use v16 in initialize_dpi but we can toggle the head logic via a local check 
    # OR we modify initialize_dpi to accept v16.1 vs v16.2
    
    # Let's use a simpler way: I'll modify initialize_dpi to support v16.1 (No Phase 4) and v16.2 (With Phase 4)
...
def main():
    ...
    res_v16_1 = run_session("v16.1", train_loader, val_loader, device)
    torch.cuda.empty_cache(); time.sleep(2)
    res_v16_2 = run_session("v16.2", train_loader, val_loader, device)
    
    print("\n" + "="*60)
    print("🏆 GENERATIONAL DUEL: v16.1 vs v16.2 (Zero-Wait)")
    print("="*60)
    print(f"{'Step':<6} | {'v16.1 (Base)':<15} | {'v16.2 (ZW Head)'} | {'Delta'}")
    print("-" * 60)
    for s in [1, 200, 400, 600, 800, 1000]:
        diff = res_v16_2[s] - res_v16_1[s]
        print(f"{s:<6} | {res_v16_1[s]:.4f}          | {res_v16_2[s]:.4f}          | {diff:.4f}")
    print("="*60)
