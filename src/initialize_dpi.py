...
def init_phase0_embedding(model, dataloader, k_samples=2000, use_exact_svd=False):
    ...
    if use_exact_svd:
        ...
        k = min(k_samples, vocab_size); C_nk = torch.zeros(vocab_size, k, device='cpu')
        for i, batch in enumerate(dataloader):
            if isinstance(batch, (list, tuple)): x = batch[0]
            else: x = batch
            u, v = x[:, :-1].reshape(-1), x[:, 1:].reshape(-1)
            mask = (v < k); u_m, v_m = u[mask], v[mask]
            if u_m.numel() > 0: C_nk.index_put_((u_m, v_m), torch.ones_like(u_m, dtype=torch.float), accumulate=True)
            if i >= 100: break
        W = C_nk[:k, :].float(); Uw, Sw, Vw = torch.svd(W)
...
def initialize_dpi(model, dataloader, warp_zeta=1.1, spectral_gamma=0.25, morph_alpha=0.45, use_calibration=True, use_exact_svd=True, residual_scale=1.0, mlp_jitter=0.02):
    """
    Args:
        mlp_jitter: Small random noise (0.02) applied to MLPs for optimal regularization.
    """
    device = next(model.parameters()).device
    ...
    print(f"  [Phase 2] Sequential Bootstrapping (Jitter: {mlp_jitter}, ResScale: {residual_scale:.3f})...")
    dct_cache = {}
    for l in range(n_layers):
        ...
        ortho_peak = math.sin(math.pi * progress); M_k = (1-progress) * centers + progress * svd_basis; Q_k, _ = torch.linalg.qr(M_k.t())
        ...
        alignment = 0.6 * (1.0 - progress); W_q.weight.data = normalize_weight(alignment * W_k.weight.data + (1-alignment) * svd_basis)
        ...
        if l % 5 == 0 or l == n_layers - 1: print(f"    Layer {l:2d} Sculpted (Signal Energy: {S[0].item():.2f})")
